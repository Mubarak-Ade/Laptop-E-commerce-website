...
function SamsungProduct() {
    const data = Data.filter(value => value.brand === "Samsung")
        ...
        <div className=''>
            <div id='product' className="grid place-items-center items-center gap-10 grid-cols-[repeat(4,_1fr)]">
                {data.map((value, index) =>
                    <motion.div
                        whileHover={'hover'}
                        key={index} className="h-fit w-[17rem] relative flex flex-col items-center gap-2 p-2  border-[3px] rounded-2xl border-accent">
